...
export default function ProviderDashboard() {
  ...
  const handleAccept = async (taskId) => {
    setAccepting(taskId);
    try {
      const { data: task } = await api.post(`/tasks/${taskId}/accept`);
  ...
  return (
    <div style={{ height:'calc(100vh - 60px)', display:'flex', flexDirection:'column', background:'#141414', color:'#e8e8e8' }}>
      ...
      {/* Top nav */}
      <div style={{ background:'#1e1e1e', borderBottom:'1px solid #2a2a2a', padding:'0 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      ...
                      <div key={task._id} style={{borderBottom:'1px solid var(--gray-100)',padding:'14px 16px'}}>
                        ...
                        {!isExpired && (
                          <div style={{display:'flex',gap:8}}>
                            <button className="btn btn-ghost btn-sm" style={{flex:1,border:'1px solid var(--gray-200)'}} onClick={()=>handleReject(task._id)} disabled={!!accepting}>
                              Decline
                            </button>
                            <button className="btn btn-success btn-sm" style={{flex:2}} onClick={()=>handleAccept(task._id)} disabled={!!accepting}>
